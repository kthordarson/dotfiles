...
# for i in `ls /dev/sd*|grep -E '^(\/)dev(\/)sd[a-z]$'`; do smartctl -a $i | grep -E -i 'serial|Reallocated_Sector|wear_leveling'; done
...
function smartcheck()
{
	...
	# Obtain the serial
        ...
        serial=$(echo "$smartoutput" | grep -E "Serial Number:|Serial number:" | grep -v "\[No" | awk '{print $3}')
        if [ -n "${serial}" ] && [[ " ${SERIALS[*]} " == *" ${serial} "* ]]; then
	...
	# SSD Wear Level
        ...
        var=$(echo "$smartoutput" | grep -E -i "177 Wear_Leveling|231 SSD_Life_Left|^173 Un|233 Media_Wearout_" | awk '{print $4}' | sed 's/^0\|^00//' | head -n 1)
	...
	# Early Warning Raw_Read_Error_Rate
        ...
        var=$(echo "$smartoutput" | grep -E -i "1 Raw_Read_Error_Rate" | awk '{print $10}' | sed 's/^0\|^00//')
        ...
        #var=`smartctl -a $1 | grep -E "read:" | awk '{print $8}'`
        var=$(echo "$smartoutput" | grep -E "read:" | awk '{print $8}')
        if [[ $var -gt 0 ]]
        then
        ...
        #var=`smartctl -a $1 | grep -E "write:" | awk '{print $8}'`
        var=$(echo "$smartoutput"  | grep -E "write:" | awk '{print $8}')
        if [[ $var -gt 0 ]]
        then
        ...
        #var=`smartctl -a $1 | grep -E "verify:" | awk '{print $8}'`
        var=$(echo "$smartoutput"  | grep -E "verify:" | awk '{print $8}')
        if [[ $var -gt 0 ]]
        then
...
#for i in `ls /dev/sd*|grep -E '^(\/)dev(\/)sd[a-z]$'`;
for i in $(find /dev -type b -name 'sd*' | grep -E '^(\/)dev(\/)sd[a-z]$');
do
        smartcheck "$i" "$DEBUG"
        rval=$?
...
#for i in `ls /dev/pass*|grep -E '^(\/)dev(\/)pass[0-9]+$'`;
for i in $(find /dev -type c -name 'pass*' | grep -E '^(\/)dev(\/)pass[0-9]+$');
do
        smartcheck "$i" "$DEBUG"
        rval=$?
        agerror=$((agerror + rval))
...
#for i in `ls /dev/sg*|grep -E '^(\/)dev(\/)sg[0-9]+$'`;
for i in $(find /dev -type c -name 'sg*' | grep -E '^(\/)dev(\/)sg[0-9]+$');
do
        smartcheck "$i" "$DEBUG"
        rval=$?
        agerror=$((agerror + rval))
