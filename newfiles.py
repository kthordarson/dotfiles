...
def filelist_generator(startpath):
	...
	filelist_ = [FileItem(Path(k)) for k in glob.glob(str(startpath),recursive=True, include_hidden=True)]
		...
		try:
			if Path(file).is_file() and not Path(file).is_symlink():
				for p in file.parts:
					...
					yield((Path(file), file.stat().st_size, file.stat().st_ctime))
...
if __name__ == '__main__':
	...
	if args.path.endswith('/'):
		...
		startpath = args.path + '/**'
	#filelist = [k for k in filelist_generator(args.path)]
